...
#### Already ordered but re-do just to check (NEEDS TO ADAPT TO INPUT) #### 
...
# Load human data
dataDir <- "/shared/hidelab2/shared/Sokratis/pathprint/Fingerprinting/data"
header<-"UnionDPD_"
load(paste(dataDir, "/", header, ".human.cor.matrix.",
           "2017-08-17", ".RData",
           sep = ""
))
...
# Create CMAP network with all Pathways
library(GeneNet)
...
dataDir <- "/data/mdt16kk/Parkinsons"

# save(DPD.pcor.est, file = paste(dataDir, "DPD.pcor.est.RData", sep = "/"))
...
save(DPD.pcor.est.results, file = paste(dataDir, "DPD.pcor.est.results.RData", sep = "/"))
# load(paste(dataDir, "DPD.pcor.est.results.RData", sep = "/"))

dataDir <- "/data/mdt16kk/Parkinsons/patient_control_DMSO"
# want to know
...
if (2 == 2){
  ...
  { 
    ...
    write.csv(attribute.table,
              file = networkAttributesOutputFile, quote = F)
    
  }
  
}

# try to create composite CMAP score by combining p-value for positive and negative correlation
...
# need to split into up and down
CMAPtestnodes <- match(CMAPnamesUse, nodeNames)
