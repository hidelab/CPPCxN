...
setwd("/Users/kkoler/Google Drive/Documents/PDN/pathway_analysis/")

# Set project files
project_outer_dir <- "pathway_analysis_test"
project_inner_folder <- "local"
# inner data
pathwayDataFilename <- "sigpathways_test.csv"
# absolute path
humanFilename <-"sub_square_imporved_PCxN_MSigDB.L1000CDS2.subset.RDS"
genesetFilename <- "/Users/kkoler/Google Drive/Documents/PDN/LINCSCDgenesets/MSigDB.L1000CDS2.subset.RDS"

#versions of DPD.pcor.est data
DPD.pcor.est.date <- "2018-10-03"
...
# threshold <- 3
#lowering the treshold for test set!!!
threshold <- 3 #test
...
# genesetData <- readRDS(paste("data/", genesetFilename, sep=""))
genesetData <- readRDS(genesetFilename)
...
ClusterA[!(ClusterA %in% names(genesetData))]
ClusterB[!(ClusterB %in% names(genesetData))]
ClusterC[!(ClusterC %in% names(genesetData))]
ClusterD[!(ClusterD %in% names(genesetData))]

length(ClusterA) == sum(sub("^Pathway.", "", names(genesetData)) %in% ClusterA) #the geneset data has a prefix of Pathway.
length(ClusterB) == sum(sub("^Pathway.", "", names(genesetData)) %in% ClusterB)
length(ClusterC) == sum(sub("^Pathway.", "", names(genesetData)) %in% ClusterC)
length(ClusterD) == sum(sub("^Pathway.", "", names(genesetData)) %in% ClusterD)
...
sum(is.na(cor.matrix))/(nrow(cor.matrix)^2) #check how many are NA
cor.matrix[is.na(cor.matrix)] <- 0
cor.matrix <- `class<-`(cor.matrix, 'numeric')

#### Just use highest concentration of CMPA drug ####	 
CMAPnames <- rownames(cor.matrix)[grepl("L1000CDS2.", rownames(cor.matrix))]
CMAPdrug <- sapply(CMAPnames, function(x){unlist(sub(".*?:(.*?):.*?$", "\\1", x))})
CMAPconcentration <- sapply(CMAPnames, function(x){unlist(sub(".*?:(.*?):(.*?)$", "\\2", x))})
CMAPcell <- sapply(CMAPnames, function(x){unlist(sub(".*?_(.*?)_.*?$", "\\1", x))})
CMAPdirection <- sapply(CMAPnames, function(x){unlist(sub("^.*?\\.(.+?)(\\..*?$)", "\\1", x))})
...
####Load or Create CMAP network with all Pathways ####
if(file.exists(paste("../../data/",humanFilename,".pcor.est.", DPD.pcor.est.date,".RData", sep = ""))){ #check where it is getting saved
  ### Loading GeneNet object if already exists 
  # load(paste(dataDir, "DPD.pcor.est.RData", sep = "/"))
  # load("DPD.pcor.est.RData")
  load(paste("../../data/",humanFilename,".pcor.est.", DPD.pcor.est.date,".RData", sep = ""))
  print("yaay")
} else {
  DPD.pcor.est <- ggm.estimate.pcor(cor.matrix[
  ...
  head(DPD.pcor.est)
  head(rownames(DPD.pcor.est))
  # save(DPD.pcor.est, file = "DPD.pcor.est.RData")
  save(DPD.pcor.est, file = paste("../../data/",humanFilename,".pcor.est.", Sys.Date(),".RData", sep = ""))
}
...
if(file.exists(paste("../../data/",humanFilename,".pcor.est.results.", DPD.pcor.est.date,".RData", sep = ""))){
  # load("DPD.pcor.est.results.RData")
  load(paste("../../data/",humanFilename,".pcor.est.results.", DPD.pcor.est.date,".RData", sep = ""))
} else {
  #### Create network from GeneNet object ####
  ...
  save(DPD.pcor.est.results, file = paste("../../data/",humanFilename,".pcor.est.results.", Sys.Date(),".RData", sep = ""))
...
if (2 == 2){
  ...
  DPD.pcor.est.net.0.9 <- extract.network(DPD.pcor.est.results, cutoff.ggm=0.1) #test
  #test fails if the cutoff too high
  ...
  for (i in 1:length(testClusters))
  {
    ...
    includeNodeNames <- union(includeNodeNames, rownames(scoreData)[scoreData[,1] >= threshold]) #test that it is the same if you put 3 here or up.	
  ...
  { 
    ...
    testNetwork.0.9$NodeType1[grep("L1000CDS2.up", testNetwork.0.9$node1Name)] <- "L1000CDS2_UP"
    testNetwork.0.9$NodeType1[grep("L1000CDS2.down", testNetwork.0.9$node1Name)] <- "L1000CDS2_DOWN"
    ...
    testNetwork.0.9$NodeType2[grep("L1000CDS2.up", testNetwork.0.9$node2Name)] <- "L1000CDS2_UP"
    testNetwork.0.9$NodeType2[grep("L1000CDS2.down", testNetwork.0.9$node2Name)] <- "L1000CDS2_DOWN"
    ...
    attribute.table$NodeType[grep("L1000CDS2.up", attribute.table$NodeName)] <- "L1000CDS2_UP"
    attribute.table$NodeType[grep("L1000CDS2.down", attribute.table$NodeName)] <- "L1000CDS2_DOWN"
...
rownames(CMAPlookup) <-gsub("L1000CDS2.up.", "", gsub(".up", "", CMAPnamesUse[grep("L1000CDS2.up", CMAPnamesUse)]))
CMAPlookup$MasterIndx <- 1:nrow(CMAPlookup)
# CMAPlookup$UPindx <- match(paste("L1000CDS2.up.", paste(rownames(CMAPlookup), ".up", sep = ""), sep = ""), nodeNames)
# CMAPlookup$DOWNindx <- match(paste("L1000CDS2.down.", paste(rownames(CMAPlookup), ".down", sep = ""), sep = ""), nodeNames)
#test?
CMAPlookup$UPindx <- match(paste("L1000CDS2.up", sub("L1000CDS2", "", rownames(CMAPlookup)), sep = ""), nodeNames)
CMAPlookup$DOWNindx <- match(paste("L1000CDS2.down", sub("L1000CDS2", "", rownames(CMAPlookup)), sep = ""), nodeNames)
...
#### Examine sample node --- NEEDS CHANGING ####
# combined
CMAPlookup[grep("CPC004_MCF7_6H:BRD-K39120595:10.0", rownames(CMAPlookup)),
           colnames(CMAPlookup) %in% paste(testClusters[[4]],"_neg",sep = "")]
CMAPlookup[grep("CPC004_MCF7_6H:BRD-K39120595:10.0", rownames(CMAPlookup)),
           colnames(CMAPlookup) %in% paste(testClusters[[4]],"_pos",sep = "")]

# original
temp1 <- CMAP.results[CMAP.results$node2 %in% grep("CPC004_MCF7_6H:BRD-K39120595:10.0",nodeNames) & 
...
# create single combined score from the 2 p-values
ClusterCMAPpvals$AposCneg <- pchisq( (-log(ClusterCMAPpvals$ClusterA.Pos)) + 
                                       (-log(ClusterCMAPpvals$ClusterC.Neg)),2,lower.tail = F)
ClusterCMAPpvals$AnegCpos <- pchisq( (-log(ClusterCMAPpvals$ClusterA.Neg)) + 
                                       (-log(ClusterCMAPpvals$ClusterC.Pos)),2,lower.tail = F)

# now order by negative correlation to each of the pathways
# write.csv(ClusterCMAPpvals[order(ClusterCMAPpvals$AnegCpos, decreasing = F),], file = "ClusterCMAPpvals.csv") #add the right naming to the file.... maybe there is some below.
...
write.csv(ClusterCMAPpvals[order(ClusterCMAPpvals[,paste(names(testClusters)[1], ".CombinedScore", sep = "")], decreasing = F),],file = paste( prefix,"ClusterCMAPpvals", Sys.Date(),"csv", sep = "."))
