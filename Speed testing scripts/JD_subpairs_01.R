JD_subpairs_01 <- function(lower,upper) {
    ...
    ProcessElement = function(ic){
        ...
        if (startsWith(names(gs_lst[i]), "Pathway.") & startsWith(names(gs_lst[j]), "Pathway.")) {
            return(NULL)
        }
        else {
            # shared genes
    ...
    for(j in seq_along(tissue_series)){
        ...
        # Excluding pathway-pathway pairs
        nms <- lapply( nms, function(x) if(check_path_path(x)) {return(NULL)} else return(x))
        nms[sapply(nms, is.null)] <- NULL
        
        ll <- combn( gs_lst , 2 , simplify = FALSE )
        for(ln in 1:length(ll)) {
            n1 <- names(ll[[ln]])[1]
            n2 <- names(ll[[ln]])[2]
            
            if(startsWith(n1, "Pathway.") & startsWith(n2, "Pathway.")){
                ll[ln] <- list(NULL) 
            }
        }
        ...
        saveRDS(res, paste0(output_folder,"/mean_pcor2_barcode/DJ_subpair_",fname,"_cpad_pathcor.RDS"))
    }
}