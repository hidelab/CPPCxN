og_subpairs_01 <- function(lower,upper) {
    ...
    ProcessElement = function(ic){
        ...
        if (startsWith(names(gs_lst[i]), "Pathway.") & startsWith(names(gs_lst[j]), "Pathway.")) {
            ...
            summaryA = GetSummary(dat=exprs_rnk,gs=gsA,colMeans)
            summaryB = GetSummary(dat=exprs_rnk,gs=gsB,colMeans)
    ...
    for(j in seq_along(tissue_series)){
        ...
        saveRDS(res, paste0(output_folder,"/mean_pcor2_barcode/O_subpair_",fname,"_cpad_pathcor.RDS"))
    }
}